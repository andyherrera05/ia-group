            ...
            <div class="space-y-4">
                ...
                <div class="bg-white/5 border border-white/10 rounded-xl p-4 animate-fade-in">
                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex justify-between items-center">
                        Lista de Productos
                        <span class="bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded text-[10px]">{{ count($items) }} items</span>
                    </h4>
                    <div class="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-1">
                        ...
                        <div class="flex items-center bg-black/30 p-2 rounded-lg border border-white/5 hover:border-yellow-500/30 transition-colors group">
                            ...
                            <div class="flex-1 min-w-0 mr-2">
                                <h5 class="text-white text-sm font-medium truncate">{{ $item['producto'] ?? 'Producto' }}</h5>
                                <div class="flex flex-wrap gap-2 text-[10px] text-gray-400 mt-0.5">
                                    <span class="text-yellow-500/80">{{ $item['cantidad'] }} uds</span>
                                    <span>•</span>
                                    <span>${{ number_format($item['valor_unitario'] ?? ($item['valorMercancia'] ?? 0), 2) }} c/u</span>
                                    <span>•</span>
                            ...
                            <div class="text-right flex flex-col justify-center">
                                @php
                                    $totalVal = isset($item['total_valor']) ? $item['total_valor'] : (($item['valorMercancia'] ?? 0) * $item['cantidad']);
                                    $totalPes = isset($item['total_peso']) ? $item['total_peso'] : (($item['peso'] ?? 0) * $item['cantidad']);
                                @endphp
                                <span class="text-white font-mono text-xs font-bold">${{ number_format($totalVal, 2) }}</span>
                ...
                <div class="bg-white/5 backdrop-blur-xl border border-yellow-500/20 rounded-2xl p-5 shadow-xl">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex gap-3" style="align-items: center;">
                            ...
                            <div style="width: 100px; height: 100px; min-width: 100px; min-height: 100px;" class="shrink-0 relative group">
                            ...
                            <div class="flex-1 flex flex-col gap-2">
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    <div>
                                        <input type="number" wire:model="temp_cantidad" placeholder="Cant." class="w-full h-[42px] p-3 bg-black/40 border border-yellow-500/10 rounded-lg text-white text-xs focus:border-yellow-500/50 focus:outline-none placeholder-gray-600">
                                        @error('temp_cantidad') <span class="text-red-400 text-[10px] block mt-0.5">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <input type="number" wire:model="temp_valor_unitario" placeholder="Valor ($)" class="w-full h-[42px] p-3 bg-black/40 border border-yellow-500/10 rounded-lg text-white text-xs focus:border-yellow-500/50 focus:outline-none placeholder-gray-600">
                                        @error('temp_valor_unitario') <span class="text-red-400 text-[10px] block mt-0.5">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <input type="number" wire:model="temp_peso_unitario" placeholder="Peso (Kg)" class="w-full h-[42px] p-3 bg-black/40 border border-yellow-500/10 rounded-lg text-white text-xs focus:border-yellow-500/50 focus:outline-none placeholder-gray-600">
                                        @error('temp_peso_unitario') <span class="text-red-400 text-[10px] block mt-0.5">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-1 gap-3 items-center mt-2">
                            ...
                            <div class="bg-black/20 rounded-lg p-2 border border-white/5 relative">
                                <label class="text-[10px] text-gray-500 block mb-2 text-center">Dimensiones Unitarias (cm)</label>
                                <div class="flex gap-2">
                                    <div class="flex-1">
                                        <label class="block text-[8px] text-gray-500 text-center mb-1 uppercase">Largo</label>
                                        <input type="number" wire:model="temp_largo" placeholder="0" class="w-full bg-transparent border-b border-gray-700 text-center text-xs text-white focus:border-yellow-500 outline-none pb-1">
                                    </div>
                                    <div class="flex-1">
                                        <label class="block text-[8px] text-gray-500 text-center mb-1 uppercase">Ancho</label>
                                        <input type="number" wire:model="temp_ancho" placeholder="0" class="w-full bg-transparent border-b border-gray-700 text-center text-xs text-white focus:border-yellow-500 outline-none pb-1">
                                    </div>
                                    <div class="flex-1">
                                        <label class="block text-[8px] text-gray-500 text-center mb-1 uppercase">Alto</label>
                                        <input type="number" wire:model="temp_alto" placeholder="0" class="w-full bg-transparent border-b border-gray-700 text-center text-xs text-white focus:border-yellow-500 outline-none pb-1">
                                    </div>
                                </div>
                            </div>

                            <!-- Action Button -->
                            <button wire:click="agregarProducto" class="w-full p-4 h-[48px] bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 text-black font-bold text-xs rounded-lg shadow-lg hover:shadow-yellow-500/20 transition-all flex items-center justify-center uppercase tracking-wider">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
